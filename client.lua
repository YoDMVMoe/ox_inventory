...
-- RedM inventory focus guard.
-- NUI receives mouse and keyboard input while gameplay controls are suppressed.
...
local function canOpenInventory()
    ...
    if shared.framework == 'vorp' then
        ...
        isDead = IsPedFatallyInjured(playerPed)
    end

    if PlayerData.dead or isDead then
        return shared.info('cannot open inventory', '(fatal injury)')
    end

    if PlayerData.cuffed
        or (
            shared.framework ~= 'vorp'
            and IsPedCuffed(playerPed)
        )
    then
...
local defaultInventory = {
    ...
    -- RedM ground drops are not implemented yet.
    slots = shared.framework == 'vorp' and 0 or shared.dropslots,

    weight = 0,

    -- Prevent moving items into the GTA-style ground inventory.
    maxWeight = shared.framework == 'vorp' and 0 or shared.dropweight,
...
function client.openInventory(inv, data)
    ...
    if shared.framework ~= 'vorp' and not cache.vehicle then
        if inv == 'player' then
    ...
    -- Give both keyboard and mouse focus exclusively to the inventory NUI.
    SetNuiFocus(true, true)
    SetNuiFocusKeepInput(false)
    print(('[ox_inventory][RedM] NUI opened. focused=%s'):format(tostring(IsNuiFocused())))
...
function OnPlayerData(key, val)
    if key ~= 'groups' and key ~= 'ped' and key ~= 'dead' then
        return
    end

    if shared.framework == 'vorp' then
        if key == 'dead' and val then
            currentWeapon = nil
...
-- VORP/RedM uses CFX raw keymaps because RegisterKeyMapping
-- is not currently functional on standard RedM builds.

local VORP_INVENTORY_KEY = 0x4B
local vorpInventoryInputRegistered = false

local function toggleVorpInventory()
    print(
        ('[ox_inventory][VORP] Inventory input received. loaded=%s invOpen=%s')
            :format(
                tostring(PlayerData.loaded),
                tostring(invOpen)
            )
    )
    ...
    if IsNuiFocused() then
        print(
            '[ox_inventory][VORP] Inventory opening blocked because another NUI has focus.'
        )
    ...
    if not PlayerData.loaded then
        print(
            '[ox_inventory][VORP] Inventory cannot open because the character inventory has not loaded.'
        )
    ...
    if invOpen == nil then
        print(
            '[ox_inventory][VORP] Inventory cannot open because inventory state is unavailable.'
        )
...
local function registerVorpInventoryInput()
    if vorpInventoryInputRegistered then
        return
    end

    vorpInventoryInputRegistered = true
    ...
    print(
        '[ox_inventory][VORP] Raw inventory key registered: K / virtual-key 0x4B.'
    )
end

if shared.framework == 'vorp' then
    registerVorpInventoryInput()
end
...
function client.closeInventory()
    ...
    if invOpen then
        ...
        -- Release NUI mouse and keyboard focus.
        SetNuiFocusKeepInput(false)
        SetNuiFocus(false, false)
        print(('[ox_inventory][RedM] NUI closed. focused=%s'):format(tostring(IsNuiFocused())))
...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
    ...
    PlayerData.maxWeight =
        tonumber(player.maxWeight)
        or shared.playerweight

    PlayerData.slots =
        tonumber(player.slots)
        or shared.playerslots
    ...
    client.interval = SetInterval(function()
        if shared.framework == 'vorp' then
            ...
            if invOpen == true and not canOpenInventory() then
                client.closeInventory()
            end
                        ...
                        if IsPedShooting(playerPed) then
                            ...
                            if currentAmmo <= 0 then
                                ...
                                currentWeapon.timer = GetGameTimer() +
                                    (GetWeaponTimeBetweenShots(currentWeapon.hash) * 1000) +
                                    100
                            end
                        end
    ...
    client.player:set('canUseWeapons', shared.framework ~= 'vorp')
